...
function loadWrongQuestions(){
  ...
  html+=`<button onclick="practiceAgain(this)" class="practice-btn practice-again-btn" data-text='${JSON.stringify(q.audioText||q.question).replace(/'/g,"&#39;")}'>🔁 Phir Se Practice</button>`;
...
function practiceAgain(btn){
  const qText = btn.getAttribute('data-text'); // button se text nikal lo
  const all = JSON.parse(localStorage.getItem('wrongQuestions')) || [];
  const q = all.find(item => (item.audioText || item.question) === qText); // text se dhoondo
