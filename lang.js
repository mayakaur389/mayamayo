...
function loadWrongUI(lang){
  ...
  if(!data) return; // agar words.js load nahi hua to skip
  ...
  setText('btn_back', "⬅ " + data.btn_back);
  setText('total_text', data.total + ": 0 " + data.questions_text);
  setText('no_question_text', data.no_question);
  setText('btn_lessons', data.lessonsBtn);
  setText('btn_speaking', data.practiceBtn);
  setText('btn_listening', data.listenBtn);
}
